...
router.post ('/:id/', (req, res) => {
  console.log (
    `POST request from ${Buffer.from (req.params.id, 'base64').toString ('utf-8')}`
  );
  // Input validation
  ...
  // Parses POST request
  req.on ('end', async () => {
    const discordId = body.substring (
      body.indexOf ('=') + 1,
      body.indexOf ('&')
    );
    const MoodleToken = body.substring (
      body.lastIndexOf ('=') + 1,
      body.length
    );
    console.log (
...
router.delete ('/:id', async (req, res) => {
  ...
  if (!discordId || discordIdValRes !== true) {
    res
      .status ('400')
      .send (`Empty or incorrect Discord id and/or moodle token`);
    return;
  }
