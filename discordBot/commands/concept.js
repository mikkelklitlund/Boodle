                ...
                if (res) {
                    ...
                    await axios.get("https://www.moodle.aau.dk/webservice/rest/server.php", {
                        ...
                        if (res.data.errorcode) {  //If moodle couldn't find any data related to the token    
                            ...
                            for (let i = 0; i < interaction.options.get("input").value; i++) {
                                courselist += `Course: ${res.data.events[i].course.fullname}\nLocation: ${res.data.events[i].location}\nTime: ${timeConverter(res.data.events[i].timestart)}\nLink: ${res.data.events[i].course.viewurl}\n\n`;
                            }
